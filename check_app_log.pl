...
my $interval = 1;
			...
			$pattern = 'response2Client.*"mid":"\d+","code":"0+"';#'reponse2Client.*"mid":"\d+","code":"0+"';
			$pattern2 = 'response2Client.*"mid":"\d+","code":"(0[2-9]|1)"';#'reponse2Client.*"mid":"\d+","code":"(0[2-9]|1)"';
			$pattern3 = 'Exception|timeout|ERROR';
			$pattern_vcb ='(.*)\s-\s\[\d+\]\s-\s\[process\]\s-\s+INFO\sBusinessManager:212\s-\srequestTime:\s(\d+)'; 
			#2018-02-26T17:37:54,557 - [151964146867087] - [process] -  INFO BusinessManager:212 - requestTime: 5886
	...
	if ($File::Find::name =~ m/$logfile$/ ) { # && (-T) match only files that are ASCII files (-T) and that contain the file name
		...
		if ((-M)*1440-(360) < $interval+$timeout_trans) 
			...
			while (defined($line = $LOGS->readline) ) {
				$line_no++;
				eval {
                     $thamso_json = parse_json($line);
                     };
                if ($@) {
                    warn $@;
                       }
               $dateString = $thamso_json->{"timestamp"};
				...
				eval
				{
					...
					if ($dt_tzadjusted > ($oldestDate)) { #Khoang thoi gian tim End
						...
						if ($line =~ m/$pattern_vcb/){
							...
							$time_trans = ($2+0)/1000;
							$time_trans_total = $time_trans_total + $time_trans;
							#Tinh min max
...
if ($hits3 > ($critical + 0)) {
	...
	printf "Trans_time_avg=%.1fs;%.1f;%.1f;%.1f;%.1f; ",$time_trans_avg,$timeout_trans*0.5,$timeout_trans,$time_trans_min,$time_trans_max;
	printf "Trans_time_min=%.1fs ",$time_trans_min;
...
if ($time_trans_avg > $timeout_trans*0.7){
